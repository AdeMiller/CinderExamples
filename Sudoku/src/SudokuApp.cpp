...
public:
  SudokuApp()
      : board_offset(50.0f, 50.0f), sqr_size(90.0f), num_size(sqr_size / 3.0f),
        num_mid(num_size / 2.0f), blk_size(num_size * 3.0f),
        blk_mid(blk_size / 2.0f), big_font("Courier New", 42.0f),
        sml_font("Courier New", 16.0f),

        solver(), puzzles(), puzzle(0), is_dirty(false), run_mode(false),
        run_timer(true) {}
...
void SudokuApp::setup() {
  setWindowSize(board_offset.x * 2 + sqr_size * kGridSize,
                board_offset.y * 2 + sqr_size * kGridSize);

  gl::clear(ColorA::black());

  puzzles = {"09000500780007000101089004007004000600896720020001007002005906040"
             "0080003500400010",
             "30060008098701400020003010007000650050040900600930002000804000500"
             "0760918050003002",
             "97...6.5...67..21.....5...668......7..5...9..7......414...7....."
             "37..26...2.5...73",
             ".164.....2....9...4......62.7.23.1..1.......3..3.87.4.96......5.."
             ".8....7.....682.",
             "........74.6..7.....71285.6..3.71.5.8.......3.1.84.2..6.89327...."
             ".4..9.51........",
             "964.........6..1......7.5...8.9.3...25......63...4...7.....4....."
             "25...4.6..8....3",
             "9672415832.......64.......98.......57958321641.......26.......75."
             "......1321756498",
             "97.....5.6..5.8.3....6..748...3...2..6.....9..1...9...187..5...."
             "3.2.7..4.2.....73",
             "97....4.8...1....5....54.....98....414.....763....19.....67....4."
             "...9...7.6....91",
             "97...483..5..8......631...........2.7..925..8.4...........672...."
             "..4..9..351...76",
             "97...6.....4..8...1.2...6..3...9...5428...31.7...3...6..6...7.1.."
             ".2..4.....1...58",
             "97..581.....9...2..2...6.5.7...82.6.....7.....1.39...2.4.8...1.."
             "5...3.....156..79",
             "97.1..4.2...7...9.......761....6.1...85...94...7.9....893......."
             "1...5...7.4..9.36",
             "................................................................."
             "...............",
             "................................................................."
...
void SudokuApp::draw_board() const {
  for (auto x = 0; x <= kGridSize; ++x) {
    ...
    gl::drawLine(board_offset + ivec2(x * sqr_size, 0.0f),
                 board_offset + ivec2(x * sqr_size, sqr_size * 9.0f));
    gl::drawLine(board_offset + ivec2(0.0f, x * sqr_size),
                 board_offset + ivec2(sqr_size * 9.0f, x * sqr_size));
  }
  gl::drawString("Move: " + to_string(solver.moves()),
                 board_offset + ivec2(0, -num_size), ColorA(1, 1, 1), sml_font);
}
...
void SudokuApp::draw_value(const int row, const int col,
  ...
  gl::drawStringCentered(
      to_string(cell_value(cell)),
      board_offset + ivec2(blk_mid + sqr_size * col, blk_mid + sqr_size * row),
      color, big_font);
}

void SudokuApp::draw_values(const int row, const int col,
                            const Cell &cell) const {
  auto offset =
      board_offset + ivec2(num_mid + sqr_size * col, num_mid + sqr_size * row);
  for (auto i = 0; i < kGridSize; ++i) {
    if (cell & (0b1 << i))
      gl::drawStringCentered(to_string(i + 1),
                             offset +
                                 ivec2(num_size * (i % 3), num_size * (i / 3)),
